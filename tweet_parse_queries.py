...
unique_index = pd.Index(list(tweets))
keys = []
for x in data:
    index_val = unique_index.get_loc(str(x["key"]))
    obj = {
        "index": index_val,
        "key": str(x["key"])
    }
...
tweets['park'] = pd.Series(park)

# Create a dataset that will be used in a graph of tweet count by park
parks = []
tweets_by_park = []
for x in data:
    parks.append(str(x['key']))
    tweets_by_park.append(tweets[str(x['key'])].value_counts()[True])
