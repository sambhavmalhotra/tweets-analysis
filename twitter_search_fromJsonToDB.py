...
class TwitterSlicer(TwitterSearch):
    ...
    def save_tweets(self, tweets):
        ...
        tweets_to_save = []
        for tweet in tweets:
            # Lets add a counter so we can count thee max number of tweets
            self.counter += 1
            if tweet['created_at'] is not None:
                tweet['sentiment'] = get_tweet_sentiment(tweet['text'])
                tweet["group_key"] = current_key
...
if __name__ == '__main__':
    ...
    with open('queries.json') as f:
        ...
        for x in data:
            search_string = ""
            for y in x["queries"]:
                # search_query will look like "(#LegoLandMalaysia) OR (#LegoLand AND #Malaysia)"
                search_string += (" (" + y + ") " + "OR")
            search_string = search_string[:-3]
            current_key = str(x["key"])

            rate_delay_seconds = 0
            error_delay_seconds = 5

            # # Example of using TwitterSearch viz. without time
            # twit = TwitterSearchImpl(rate_delay_seconds, error_delay_seconds, None)
            # twit.search(search_string)
            ...
            # print("TwitterSlicer collected %i" % twit_slice.counter)
            # print("String: %s" % search_string)
